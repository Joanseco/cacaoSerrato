...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.scale = this.screen.height / 1300;
...
class App {
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    // Adaptamos para leer 'src' y 'alt' directamente
    ...
    this.medias = this.mediasImages.map((data, index) => {
      return new Media({
        ...
        gl: this.gl,
        image: data.src, // Cambiado de data.image a data.src
        index,
        length: this.mediasImages.length,
        renderer: this.renderer,
        scene: this.scene,
        screen: this.screen,
        text: data.alt, // Cambiado de data.text a data.alt
...
export default function CircularGallery({
  ...
  useEffect(() => {
    // Si no pasas items, usamos tus fotos por defecto aquí
    const photoSlides = items || [
  ...
  return (
    <div 
      ...
      ref={containerRef} 
      style={{ minHeight: '500px' }} // Asegura visibilidad inicial
    />
  );
}